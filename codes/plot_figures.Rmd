...
ggdensity(df,x="width",y = "density",color = "peakwidth", fill="peakwidth", alpha=0.1, palette=mypal,add="median") + scale_x_continuous(limits = c(100,800)) + ggtitle("Peakwidth CTCF_and_G4 peaks")
ggsave(glue("{plot_folder}Density_CTCF_peaks.pdf"),
       last_plot())
```
```{r}
df <- rbind(data.frame(ctcf.anno[ctcf.anno$class=="CTCF_and_G4",0:0],peakwidth="original", row.names = NULL), data.frame(ctcf.merged[ctcf.merged$class=="CTCF_and_G4",0:0],peakwidth="merged", row.names = NULL))
ggdensity(df,x="width",y = "density",color = "peakwidth", fill="peakwidth", alpha=0.1, palette=mypal,add="median") + scale_x_continuous(limits = c(100,800)) + ggtitle("Peakwidth CTCF_and_G4 peaks") + geom_label(data=df %>% group_by(peakwidth) %>% summarise(width=median(width)),aes(y=peakwidth,x=width,label=width) )
ggsave(glue("{plot_folder}Density_CTCF_peaks_medians.pdf"),
       last_plot())
```
```{r}
```
...
ggbarplot(as.data.frame(t(as.matrix(tb))),"Var2","Freq",fill="Var1",palette = c(mypal[1],"#aaaaaa"),label = TRUE, lab.pos="in",ylim=c(10000,13000))
...
G4.motifs <- rtracklayer::import('../data/peaks/G4H_mm10_1.75.bed')
G4.motifs$type = rep("G4",length(G4.motifs))
ol <-  bedscout::annotate_nearby_features( makeGRangesFromDataFrame(results[,1:3],na.rm=T),feat_gr = G4.motifs,name_field = "type",distance_cutoff = 500)$nearby_features
ol[is.na(ol)] <- "none"
...
ggbarplot(as.data.frame(t(as.matrix(tb))),"Var2","Freq",fill="Var1",palette = c(mypal[1],"#aaaaaa"),label = TRUE, lab.pos="in",ylim=c(500,13000))
ggsave(glue("{plot_folder}Barplot_G4motif_500dist_by_PDSquantile.pdf"),
       plot = last_plot())
```
...
G4.motifs <- rtracklayer::import('../data/peaks/G4H_mm10_1.75.bed')
G4.motifs$type = rep("G4",length(G4.motifs))
ol <-  bedscout::annotate_nearby_features( makeGRangesFromDataFrame(results[,1:3],na.rm=T),feat_gr = G4.motifs,name_field = "type",distance_cutoff = 1000)$nearby_features
ol[is.na(ol)] <- "none"
...
ggbarplot(as.data.frame(t(as.matrix(tb))),"Var2","Freq",fill="Var1",palette = c(mypal[1],"#aaaaaa"),label = TRUE, lab.pos="in",ylim=c(0,13000))
ggsave(glue("{plot_folder}Barplot_G4motif_1kbdist_by_PDSquantile.pdf"),
       plot = last_plot())
```
