...
```{r}
compare_means(deseq.lfc.pds ~ class,data = results)
```

```{r}
compare_means(deseq.lfc.pdc ~ class,data = results)
```
...
nearest_G4_stats
write_tsv(nearest_G4_stats, "pdc_G4distance_plots-statistics.tsv")
```
```{r fig.width=5, fig.height=3}
